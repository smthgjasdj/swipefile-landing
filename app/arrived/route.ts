...
export async function GET(request: NextRequest) {
  ...
  if (!clientId || !version) {
    return NextResponse.json({ error: "clientId and version are required" }, { status: 400 });
  }
  ...
  const backend = await fetch(
    `${process.env.API_URL}/m/install?${new URLSearchParams({ uuid: clientId, extension: code }).toString()}`,
    {
      method: "POST",
  ...
  // remember which install this browser is, so notes match its version
  reply.cookies.set({
    name: "clientId",
    value: clientId,
    httpOnly: true,
